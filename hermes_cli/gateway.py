...
# Hardened deployments can explicitly own the full unit. Stock Hermes must
# not rewrite such a unit during status, start, restart, install, or boot.
_SYSTEMD_OPERATOR_MANAGED_MARKER = "# Hermes-Unit-Management: operator"


def _systemd_unit_is_operator_managed(text: str) -> bool:
    return any(
        line.strip() == _SYSTEMD_OPERATOR_MANAGED_MARKER
        for line in text.splitlines()
    )
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    if _systemd_unit_is_operator_managed(installed):
        return True
