...
def _eval_ssrf_guard_active(effective_task_id: str) -> bool:
    ...
    """
    _bt = _origin()
    return (_bt._enforce_private_url_guard_on_local() or (not _cloud._is_local_backend() and not _bt._is_local_sidecar_key(effective_task_id))) and not _cloud._allow_private_urls()
