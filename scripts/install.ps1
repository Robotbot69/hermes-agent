...
function Install-HermesCommandLaunchers {
    ...
    $requiredSource = Join-Path $scriptsDir "python.exe"
    ...
    foreach ($launcher in @("hermes", "hermes-acp")) {
        if ($launcher -eq "hermes") {
            # Invoke the trusted interpreter directly; generated EXE shims may
            # be blocked by Windows Application Control on this installation.
            Remove-Item -LiteralPath (Join-Path $Destination "hermes.exe") -Force -ErrorAction SilentlyContinue
            Set-Content -LiteralPath (Join-Path $Destination "hermes.cmd") -Encoding Ascii -Value @(
                '@echo off',
                ('"' + $requiredSource + '" -m hermes_cli.main %*'),
                'exit /b %ERRORLEVEL%'
            )
            continue
        }
