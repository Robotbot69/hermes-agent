...
class TestSystemdServiceRefresh:
    def test_operator_managed_unit_is_current_and_never_refreshed(
        self, tmp_path, monkeypatch
    ):
        unit_path = tmp_path / "hermes-gateway.service"
        original = (
            "[Unit]\n"
            "# Hermes-Unit-Management: operator\n"
            "Description=Custom hardened Hermes gateway\n"
        )
        ...
        def fail_if_generated(*args, **kwargs):
            raise AssertionError("operator-managed units must not be regenerated")

        monkeypatch.setattr(gateway_cli, "generate_systemd_unit", fail_if_generated)
